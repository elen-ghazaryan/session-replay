...
class TrackingService(
  ...
  fun listSessions(limit: Int, offset: Int): SessionListDto {
    ...
    val items = sessionRepository.findRecent(cappedLimit, cappedOffset)
  ...
  fun getDetail(id: UUID): SessionDetailDto {
    ...
    val events = eventRepository.findBySessionIdOrderByTimestampAsc(id).map {
  ...
  fun getReplay(id: UUID): ReplayDto {
    ...
    val events = eventRepository.findBySessionIdOrderByTimestampAsc(id)
  ...
  @Transactional
  fun track(request: CreateTrackRequest, ipAddress: String?) {
    log.info(
      "Tracking session={} appId={} incomingEvents={}",
      request.session.id, request.session.appId, request.events.size,
    )
    val session = sessionRepository.findById(request.session.id).orElseGet {
      Session(
